...
이 문서는 의류 쇼핑몰 MVP를 Vercel에 배포하는 방법을 안내합니다.
...
- [ ] Toss Payments 개발자 계정 생성 (테스트 모드)
...
배포 전에 Supabase 데이터베이스에 마이그레이션을 적용해야 합니다:
...
3. 마이그레이션 실행 확인:
   - Table Editor에서 `categories`, `products`, `cart_items`, `orders`, `order_items` 테이블 확인

### 4. Clerk 웹훅 설정 (선택사항)

Clerk의 사용자 동기화를 위해 웹훅을 설정할 수 있습니다:

1. Clerk Dashboard → **Webhooks** → **Add Endpoint**
2. Endpoint URL: `https://your-domain.vercel.app/api/sync-user`
3. Events: `user.created`, `user.updated` 선택
4. Secret Key 저장 (필요시)
...
2. 도메인 추가
3. DNS 설정 (Vercel에서 제공하는 가이드 따르기)

### 환경별 설정

- **Production**: 프로덕션 환경 변수 설정
- **Preview**: 프리뷰 환경 변수 설정 (Git 브랜치별)
- **Development**: 개발 환경 변수 설정
...
3. **RLS 정책**:
   - 현재는 RLS를 사용하지 않지만, 프로덕션에서는 고려 필요
   - 서버 사이드에서 권한 검증 필수
...
- [Toss Payments 배포 가이드](https://docs.tosspayments.com/guides/v1/deployment)
