...
- [x] 홈페이지 (인기 상품, 카테고리별 상품 섹션)
- [x] 상품 목록 페이지 (페이지네이션, 정렬, 카테고리 필터)
- [x] 카테고리 필터링
- [x] 상품 상세 페이지
- [x] 어드민 상품 등록 (Supabase 직접, 문서화 완료)

### Phase 3: 장바구니 & 주문 (1주) ✅ 완료

- [x] 장바구니 기능 (추가/삭제/수량 변경, 실시간 업데이트)
- [x] 장바구니 사이드바 (항상 표시, 숨기기 기능)
- [x] 주문 프로세스 구현 (배송 정보 입력, 서버 사이드 검증)
- [x] 주문 테이블 연동 (주문 번호 생성, 주문 상품 저장)

### Phase 4: 결제 통합 (1주) ✅ 완료

- [x] Toss Payments 연동 (리다이렉트 방식)
- [x] 테스트 결제 구현
- [x] 결제 성공/실패 콜백 처리
- [x] 결제 완료 후 주문 상태 업데이트
- [x] 서버 사이드 합계 검증

### Phase 5: 마이페이지 (0.5주) ✅ 완료

- [x] 주문 내역 목록 조회 (주문 번호, 상태, 총액, 상품 개수)
- [x] 주문 상세 보기 (주문 정보, 배송 정보, 상품 목록, 결제 요약)

### Phase 6: 테스트 & 배포 (0.5주) ✅ 완료

- [x] 전체 플로우 E2E 점검 (체크리스트 작성 완료)
